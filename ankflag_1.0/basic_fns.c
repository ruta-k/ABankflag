...
int findbadbase (BaseParType *baseparams, int nbase, float ***basestatsarray, int bl_doflag, float mean_tolerance, float rms_tolerance, 
	...
	for (i = 0; i < 5; i++)
		temparr[i]		=	(float *) malloc (nbase * sizeof(float));	
	
	baselines	=	(int **) malloc (nbase * sizeof(int *));
	for (i = 0; i < nbase; i++) {
		baselines[i]	=	(int *) malloc (POLARRS * sizeof(int));	
		for (p = 0; p < POLARRS; p++)
			baselines[i][p]	=	1;	
	}
	...
	for (p = 0; p < POLARRS; p++) {
		...
		j	=	0;
		for (i = 0; i < nbase; i++) {
			if (basestatsarray[i][p][4] < 1.0) {
				temparr[0][j]	=	basestatsarray[i][p][0];	//	Mean
				temparr[1][j]	=	basestatsarray[i][p][1];	//	Median
				temparr[2][j]	=	basestatsarray[i][p][2];	//	RMS
				temparr[3][j]	=	basestatsarray[i][p][3];	//	MAD
				j++;
			}
			else
				baselines[i][p]	=	0;	
		...
		if (bl_doflag%2) {
			...
			for (i = 0; i < nbase; i++) {
				...
				if (basestatsarray[i][p][1] > (meanval + outlim*rmsval*mean_tolerance*sqrt(maxize/((float) (baseparams[i].chansize*baseparams[i].recsize))))) {
					printf ("Baseline	%d %d	Pol	%d	median flagged...\n",baseparams[i].anta,baseparams[i].antb,p);
					baselines[i][p]	=	0;
				}			
			}
			...
			for (i = 0; i < nbase; i++) {
				...
				if (basestatsarray[i][p][0] > (meanval + outlim*rmsval*mean_tolerance*sqrt(maxize/((float) (baseparams[i].chansize*baseparams[i].recsize))))) {
					printf ("Baseline	%d %d	Pol	%d	mean flagged...\n",baseparams[i].anta,baseparams[i].antb,p);
					baselines[i][p]	=	0;
				}			
			}
		}
		if (bl_doflag>1) {
			...
			for (i = 0; i < nbase; i++) {
				...
				if (basestatsarray[i][p][3] > (meanval + outlim*rmsval*rms_tolerance*sqrt(maxize/((float) (baseparams[i].chansize*baseparams[i].recsize))))) {
					printf ("Baseline	%d %d	Pol	%d	MAD flagged...\n",baseparams[i].anta,baseparams[i].antb,p);
					baselines[i][p]	=	0;
				}			
			}
			...
			for (i = 0; i < nbase; i++) {
				...
				if (basestatsarray[i][p][2] > (meanval + outlim*rmsval*rms_tolerance*sqrt(maxize/((float) (baseparams[i].chansize*baseparams[i].recsize))))) {
					printf ("Baseline	%d %d	Pol	%d	RMS flagged...\n",baseparams[i].anta,baseparams[i].antb,p);
					baselines[i][p]	=	0;
				}			
			}
		}			
	...
	for (i = 0; i < nbase; i++)
			fprintf(badbasefile,"%d	%d	%d	%d	%d\n",baseparams[i].anta,baseparams[i].antb,baseparams[i].exists,baselines[i][0],baselines[i][1]);
